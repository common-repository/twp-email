...
if(isset($_POST['twpemailsubmit'])){
  $wpdb->query($wpdb->prepare("update ".$wpdb->prefix."twpEmail set email = '%s', pass = '%s',fromname= '%s', host = '%s', port = %f, secure = '%s' where id = %f",sanitize_email($_POST['email']),twpencr(stripslashes_deep($_POST['password'])),stripslashes_deep($_POST['from']),stripslashes_deep($_POST['host']),$_POST['port'],stripslashes_deep($_POST['secure']), 1));
...
<form class="twpboxsetup" method="post">
  ...
  <div class="twpmailsetup">
    ...
    <input type="email" name="email" value="<?php echo esc_attr($result[0]->email) ?>" maxlength="50" required>
  </div>
  <div class="twpmailsetup">
  ...
  <div class="twpmailsetup">
    ...
    <input type="text" name="host" value="<?php echo esc_attr($result[0]->host) ?>" maxlength="50" required>
  </div>
  <div class="twpmailsetup">
