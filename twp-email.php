<?php  /*
  ...
  Plugin URI: https://wordpress.org/plugins/twp-email/
  Description: simple smtp mail setup for wordpress
  Author: Tiago Anastácio
  Author URI: https://ittca.eu
  Version: 1.3.7
  Tags: twp, email, smtp
...
if(!function_exists('twped_csm')){
  function twped_csm($slug){
    global $submenu;if(isset($submenu['twp_editor'])){foreach($submenu['twp_editor'] as $subm){if($subm[2]==$slug)return 1;}}
  }
}
...
if(!function_exists('twpe_activation')){
  function twpe_activation(){
    global $wpdb;
    $chck = $wpdb->query('select id from '.$wpdb->prefix.'twpEmail where id = 1');
    if(!$chck){
      $wpdb->query('create table if not exists '.$wpdb->prefix.'twpEmail (id tinyint, email varchar(55), pass varchar(60), fromname varchar(30), host varchar(55), port int, secure varchar(10), b varchar(15), c varchar(15), d tinyint)');
      $wpdb->query($wpdb->prepare("insert into ".$wpdb->prefix."twpEmail values(%f,%s,%s,%s,%s,%f,%s,%s,%s,%f)",1,"","","","",587,"","AES-128-ECB","TwP3m41l!%%$",0));
    }
  }
  register_activation_hook( __FILE__, 'twpe_activation' );
}
